...
#[cfg(target_arch = "aarch64")]
unsafe fn process_row_neon_gemv_bit1_58(row: usize, ctx: &vec101_context, _x_mask: &[u64]) {
...
#[doc = " # Safety"]
#[doc = " The caller must ensure that `row` is within bounds and `ctx` pointers are valid."]
pub unsafe fn process_row_neon_gemm(row: usize, ctx: &vec101_context, _x_t: &[i8], _x_mask: &[u64], _padded_batch: usize, row_sums: &mut [i32]) {
    if ctx.blocks_per_row == 0 {
        branch_unlikely();
        return;
    }
    match ctx.quant_type {
        crate::vec101_compute::types::QuantType::Bit1_58 => unsafe { process_row_neon_gemm_bit1_58(row, ctx, _x_t, _x_mask, _padded_batch, row_sums) },
    }
}
#[cfg(target_arch = "aarch64")]
unsafe fn process_row_neon_gemm_bit1_58(row: usize, ctx: &vec101_context, _x_t: &[i8], _x_mask: &[u64], _padded_batch: usize, row_sums: &mut [i32]) {
    let scale = unsafe { *ctx.s_stream.add(row) };
